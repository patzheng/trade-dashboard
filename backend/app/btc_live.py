...
def build_btc_dashboard() -> dict[str, Any]:
    ...
    try:
        bitview_history = bitview_price_history()
        market_history = btc_price_history()
        ...
        blockchain_legacy = blockchain_stats()
        bitview_blocks = bitview_block_metrics()
        mempool = mempool_fees()
        fng = fear_greed()
        try:
            binance_premium = fetch_binance_premium_index()
            binance_oi = fetch_binance_open_interest()
            binance_oi_history = fetch_binance_open_interest_history()
            binance_funding_history = fetch_binance_funding_history()
        except (HTTPError, URLError, TimeoutError, ValueError, OSError):
            binance_premium = {}
            binance_oi = {}
            binance_oi_history = []
